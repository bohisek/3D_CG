...
/*#ifdef DEFL
		...
		DOTGPU<T,128><<<dimGrid,dimBlock,params.blockSize*sizeof(T)>>>(drh, dr, dr, params.Nx, params.Ny, params.Nz);
		...
		//while (iter < 1000) {  // only testing
			...
			if (iter==1) {
				...
				bt = rhNew/rhOld;	
				AXPY<<<128,1024>>>(dp, dr, (T)1., bt, params.Nx, params.Ny);   // p = z + beta*p	
			}
...
/*#ifdef DEFL
			...
			DOTGPU<T,128><<<dimGrid,dimBlock,params.blockSize*sizeof(T)>>>(drh, dr, dr, params.Nx, params.Ny, params.Nz);   // rhoNew = <r,z>		
			cudaMemcpy(hrh, drh, params.blocks*sizeof(T), cudaMemcpyDeviceToHost);
		    rhNew = dot(hrh,params.blocks);
		}
